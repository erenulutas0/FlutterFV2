...
class ManualMockConnectivity extends Mock implements Connectivity {
  ...
  @override
  Stream<List<ConnectivityResult>> get onConnectivityChanged => Stream.value([ConnectivityResult.none]);
}
...
void main() {
  ...
  group('Offline Sync Service Tests', () {
    ...
    test('Offline Mode: Creating word adds to local DB and Sync Queue', () async {
    ...
    test('Online Mode: Creating word calls API directly (Simulated)', () async {
      ...
      expect(result!.id, lessThan(0)); // Optimistic update returns localId initially
      ...
      final queue = await localDb.getSyncQueue();
      expect(queue.isEmpty, true, reason: 'Queue should be empty for online success');
    });
