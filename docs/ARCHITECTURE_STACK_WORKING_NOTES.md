...
4. Docker icinde Piper runtime netlestirme
- Su an `GET /api/tts/status` sonucu `available=false`; container icinde calisabilir Piper binary yolu hala net degil.
- Secenek: backend'i host'ta calistirip `C:\piper\piper.exe` kullanmak veya Docker image'a Linux Piper binary dahil etmek.
